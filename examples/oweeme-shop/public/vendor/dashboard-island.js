...
// packages/vue-island/src/index.ts
function defineVueIsland(component) {
...
// examples/oweeme-shop/src/islands/Dashboard.ts
var Dashboard = defineComponent({
  props: {
    tasks: { type: Array, default: () => [] }
  },
  setup(props) {
    const tasks = ref(props.tasks.map((t) => ({ ...t })));
    const pending = computed2(() => tasks.value.filter((t) => !t.done).length);
    const toggle = (id) => {
      const task = tasks.value.find((t) => t.id === id);
      if (task) task.done = !task.done;
    };
    return () => h("div", { class: "nx-card" }, [
...
// examples/oweeme-shop/src/islands/dashboard.island.ts
var dashboard_island_default = defineVueIsland(Dashboard);
export {
  dashboard_island_default as default
};
